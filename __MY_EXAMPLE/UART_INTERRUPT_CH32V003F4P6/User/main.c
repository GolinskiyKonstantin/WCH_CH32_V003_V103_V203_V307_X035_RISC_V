...
// указываем сколько байт будем принимать
#define SIZE_BYTE   3
...
void USART_config_1(void)
{
    ...
    // перечень прерываний ( нужное добавить через побитовый или |
    // но может не работать тогда отдельно вторым вызовом USART_ITConfig
...
//        USART_IT_ERR - Error interrupt.
    USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);          // прерывание только на прием USART_IT_RXNE
...
int main(void)
{
    ...
    char start_msg[] = "START INTERRUPT UART !!!\r\n";
    UART_Transmit(start_msg, strlen(start_msg));

    while(1)
    {
        // если данные были приняты ( SIZE_BYTE байта - для примера )
        // если кол-во байт будет принято больше чем указаные SIZE_BYTE то в буфер запишуться ПОСЛЕДНИИ SIZE_BYTE
        // если меньше то флаг не отработает будет ждать пока недостающие попадут
        if( flag_RX_END == 1 )
        {
            flag_RX_END = 0;
            UART_Transmit((uint8_t*)uart_buff, SIZE_BYTE);
        }
